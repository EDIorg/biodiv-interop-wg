#-----------------------------------------------------------------------------
# This function converts source dataset "knb-lter-fce.1203" (archived in the EDI
# Data Repository) to ecocomDP dataset "edi.346" (also archived in EDI)
# 
# Arguments:
#
...
#             "edi.346" whenever new data are added to the source 
#             "knb-lter-fce.1203". The framework executing this maintenance 
#             routine is hosted on a remote server and jumps into action 
#             whenever an update notification is received for 
#             "knb-lter-fce.1203". The maintenance routine parses the 
#             notification to get the arguments to create_ecocomDP().
#
# Landing page to source dataset "knb-lter-fce.1203":
# https://portal.edirepository.org/nis/mapbrowse?scope=knb-lter-fce&identifier=1203
# Landing page to derived dataset "edi.346":
# https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=346
# -----------------------------------------------------------------------------
...
# Flatten source dataset --------------------------------------------------
  ...
  # Separate year out from columns and combine on species
  
  wide <- data %>% 
    tidyr::pivot_longer(cols = matches(c("thalassia", "halodule", "total")),
  ...
  flat <- wide %>%
    tidyr::pivot_longer(cols = c("thalassia", "halodule", "total"),
                        names_to = "taxon_name",
                        values_to = "value")
  
  # Add the unit and variable_name column back
  
  flat$variable_name <- "abovegroundBiomassDensity"
 
  flat$unit <- "gramPerMeterSquared"
  
  message('Note: variable_name and unit derived from metadata\n')
  
  # Sorting and arranging rows by year and location 
  
  flat <- flat %>% dplyr::arrange(year, SITENAME)
  ...
  flat$location_id <- flat %>% group_by(SITENAME, TREATMENT) %>% group_indices()
  
  # Surveys are grouped by the year they occurred in
  ...
  location <- ecocomDP::create_location(
    L0_flat = flat, 
    location_id = "location_id", 
    location_name = c("SITENAME", "TREATMENT"),
    latitude = "latitude", 
    longitude = "longitude")
  ...
  i <- variable_mapping$variable_name == 'abovegroundBiomassDensity'
  variable_mapping$mapped_system[i] <- 'The Ecosystem Ontology'
  variable_mapping$mapped_id[i] <- 'http://purl.dataone.org/odo/ECSO_00002892'
  variable_mapping$mapped_label[i] <- 'aboveground biomass density'
  ...
  dataset_annotations <- c(
    ...
    `Primary Production Carbon Flux` =
      "http://purl.dataone.org/odo/ECSO_00000015",
    `species abundance` = 
  ...
  eml <- ecocomDP::create_eml(
    ...
    script_description = 
      "A function for converting knb-lter-fce.1203 to ecocomDP",
