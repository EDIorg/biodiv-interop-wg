#-----------------------------------------------------------------------------
# This function converts source dataset "knb-lter-hbr.126" (archived in the EDI
# Data Repository) to ecocomDP dataset "edi.263" (also archived in EDI)
# 
# Arguments:
#
...
#             "edi.263" whenever new data are added to the source 
#             "knb-lter-hbr.126". The framework executing this maintenance 
#             routine is hosted on a remote server and jumps into action 
#             whenever an update notification is received for 
#             "knb-lter-hbr.126". The maintenance routine parses the 
#             notification to get the arguments to create_ecocomDP().
#
# Landing page to source dataset "knb-lter-hbr.126":
# https://portal.edirepository.org/nis/mapbrowse?scope=knb-lter-hbr&identifier=126
# Landing page to derived dataset "edi.263":
# https://portal.edirepository.org/nis/mapbrowse?scope=edi&identifier=263
# -----------------------------------------------------------------------------
...
# Flatten source dataset --------------------------------------------------
  ...
  # Only one table, no joining
  
  wide <- data$gastropods.txt
  
  wide$DATE <- lubridate::ymd(wide$DATE)

  # Gather columns, which consist of individual species, into a "species" and
  # "count" column
  
  # Append "value." to individual species columns
  ...
  # Flatten the table, but send the column names to a taxon_name variable
  ...
  # Define the type of measurement (according to metadata)
  
  flat$variable_name <- "count"
  
  # Order for aesthetic purposes
  
  flat <- flat %>% dplyr::arrange(DATE, WATERSHED)
  ...
  # Observations are made at the Site level; each Site is a different lake
  
  flat$location_id <- flat %>% group_by(WATERSHED) %>% group_indices()
  
  # Each combination of location and sample date form a sampling event
  
  flat$event_id <- flat %>% 
    group_by(month=floor_date(DATE, "month"), WATERSHED) %>% group_indices()
  ...
  elev <- xml2::xml_double(
    xml2::xml_find_all(geocov, './/altitudeMaximum'))
  
  site_full_names <- c(
    'Watershed 1',
    'Bear Brook Watershed, west of\n        Watershed 6')
  ...
  flat <- dplyr::left_join(flat, geo, by = "WATERSHED")

  # Add columns for the taxon table -------------------------------------------
  
  # Remove vestigial "N_" from the taxon name
  
  flat$taxon_name = flat$taxon_name_verbatim %>% 
    stringr::str_sub(., 3, length(.))
  ...
  # remove columns of meaningless information.
  ...
  observation_ancillary <- ecocomDP::create_observation_ancillary(
    L0_flat = flat,
    observation_id = "observation_id", 
    variable_name = c("ELEV_BAND", "CARDBOARD"))
  
  taxon_ancillary <- ecocomDP::create_taxon_ancillary(
    L0_flat = flat,
    taxon_id = "taxon_id",
    variable_name = c("taxon_name_verbatim"))
  ...
  i <- variable_mapping$variable_name == 'ELEV_BAND'
  variable_mapping$mapped_system[i] <- 'The Ecosystem Ontology'
  variable_mapping$mapped_id[i] <- 'http://purl.dataone.org/odo/ECSO_00001567'
  variable_mapping$mapped_label[i] <- 'elevation'
  
  i <- variable_mapping$variable_name == 'CARDBOARD'
  variable_mapping$mapped_system[i] <- 'The Ecosystem Ontology'
  ...
  dataset_annotations <- c(
    ...
    `calcium concentration` =
      "http://purl.dataone.org/odo/ECSO_00001772")
  ...
  eml <- ecocomDP::create_eml(
    ...
    script_description = 
      "A function for converting knb-lter-hbr.126 to ecocomDP",
